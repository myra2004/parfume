...
pyCMS
    ...
    a Python / PIL interface to the littleCMS ICC ColorCRUD Management System
    Copyright (C) 2002-2003 Kevin Cazabon
    kevin@cazabon.com
...
def createProfile(
    ...
    if colorSpace not in ["LAB", "XYZ", "sRGB"]:
        msg = (
            f"ColorCRUD space not supported for on-the-fly profile creation ({colorSpace})"
        )
        raise PyCMSError(msg)
    ...
    if colorSpace == "LAB":
        try:
            ...
            msg = f'ColorCRUD temperature must be numeric, "{colorTemp}" not valid'
